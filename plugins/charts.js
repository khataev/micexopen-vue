...
const charts = {
  ...
  ChartMan: {
    ...
    //   if (featureData) {
    //     const fiz_short = featureData
    //       .get('fiz')
    //       .get('short')
    //       .toJSON()
    //     const fiz_long = featureData
    //       .get('fiz')
    //       .get('long')
    //       .toJSON()
    //     const jur_short = featureData
    //       .get('jur')
    //       .get('short')
    //       .toJSON()
    //     const jur_long = featureData
    //       .get('jur')
    //       .get('long')
    //       .toJSON()

    //     const fizChart = new Chart(this.fizPositionCtx, {
    //       type: 'pie',
    ...
    //     const jurChart = new Chart(this.jurPositionCtx, {
    //       type: 'pie',
    ...
    drawChart2: function(row_data) {
      if (row_data) {
        if (this.fizChart) this.fizChart.destroy()

        if (this.jurChart) this.jurChart.destroy()

        const fiz_options = charts.pieOptions()
        fiz_options.title.text = 'Физические лица, %'

        const jur_options = charts.pieOptions()
    ...
    updatePeriodChartWithDataPortion: function(moment, openPositions) {
      ...
      const fiz_long_perc =
        (openPositions.position.fiz_long /
          (openPositions.position.fiz_long +
            openPositions.position.fiz_short)) *
        100
      const jur_long_perc =
        (openPositions.position.jur_long /
          (openPositions.position.jur_long +
            openPositions.position.jur_short)) *
        100
      const total_long_perc =
        ((openPositions.position.fiz_long + openPositions.position.jur_long) /
          (openPositions.position.fiz_long +
            openPositions.position.fiz_short +
            openPositions.position.jur_long +
            openPositions.position.jur_short)) *
      ...
      const fiz_long = openPositions.clients.fiz_long
      const jur_long = openPositions.clients.jur_long
      const total_long =
        openPositions.clients.fiz_long + openPositions.clients.jur_long
      ...
      // АБСОЛЮТНЫЕ КОНТРАКТЫ
      this.dynamicDatasets2[0].data = _.sortBy(
        this.dynamicDatasets2[0].data,
        'x'
      )
      this.dynamicDatasets2[1].data = _.sortBy(
        this.dynamicDatasets2[1].data,
        'x'
      )
      this.dynamicDatasets2[2].data = _.sortBy(
        this.dynamicDatasets2[2].data,
        'x'
      )
      ...
      if (this.openPositionsDynamicsChart) {
        ...
        this.openPositionsDynamicsChart.options.scales.xAxes[0].time.max = this.dynamicDatasets[0].data[
          this.dynamicDatasets[0].data.length - 1
        ].x
      ...
      if (this.openPositionsDynamicsChart2) {
        ...
        this.openPositionsDynamicsChart2.options.scales.xAxes[0].time.max = this.dynamicDatasets2[0].data[
          this.dynamicDatasets2[0].data.length - 1
        ].x
    ...
    drawCurrencyRatesChart: function() {
      if (this.currencyRatesChart) this.currencyRatesChart.destroy()
    ...
    drawOpenPositionsDynamicsChart: function() {
      ...
      this.openPositionsDynamicsChart = new Chart(
        this.openPositionsDynamicsCtx,
        {
          type: 'line',
          data: {
    ...
    drawOpenPositionsDynamicsChart2: function() {
      ...
      this.openPositionsDynamicsChart2 = new Chart(
        this.openPositionsDynamics2Ctx,
        {
          type: 'line',
          data: {
